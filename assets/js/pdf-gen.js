...
const PDFGen = (() => {
  ...
  // Returns PDF as Blob (used by Assinafy integration — no download triggered)
  function generateBlob(rdo, obra, criador, linhas) {
    const { jsPDF } = window.jspdf;
    // Re-run generate logic but capture output as blob instead of saving
    const tmpDoc = _buildDoc(rdo, obra, criador, linhas);
    return tmpDoc.output('blob');
  }
  ...
  function _buildDoc(rdo, obra, criador, linhas) {
    ...
    const dias_pt = ['Segunda-feira','Terça-feira','Quarta-feira','Quinta-feira','Sexta-feira','Sábado','Domingo'];
    const roles   = ['Montador','Auxiliar Montagem','Eletricista','Soldador','Encarregado','Técnico Segurança','Total'];
    const horarios = [['Início:', rdo.horario_inicio || ''],['Almoço:', rdo.horario_almoco || ''],['Término:', rdo.horario_termino || '']];
    const tempos_opt = ['Bom (sem chuva)','Chuva Leve','Chuva Extrema'];
    const status_opt = ['Iniciada','Em Andamento','Concluída'];
    const roleKeys = ['montador','auxiliar','eletricista','soldador','encarregado','tecnico_seguranca'];
    const rH = 5.5;
    ...
    for (let i = 0; i < nRows; i++) {
      ...
      text(count > 0 ? String(count) : '', cx+29, rowY+1.2, { size: 7.5, bold: true, color: dark });
      cx = ML+74;
      if (i < 3) { text(horarios[i][0], cx+1, rowY+1.2, { size: 7, bold: true, color: gray }); text(horarios[i][1], cx+15, rowY+1.2, { size: 7.5, color: black }); }
      cx = ML+116;
      if (i < 3) { const isT = tempos_opt[i] === rdo.condicao_tempo_manha || tempos_opt[i] === rdo.condicao_tempo_tarde; text((isT ? '●' : '○') + ' ' + tempos_opt[i], cx+1, rowY+1.2, { size: 7, color: black }); }
      else if (i === 3) { text('Status da Atividade', cx+1, rowY+1.2, { size: 7, bold: true, color: dark }); }
      else { const sIdx = i - 4; if (sIdx < status_opt.length) { const isS = status_opt[sIdx] === rdo.status_atividade; text((isS ? '●' : '○') + ' ' + status_opt[sIdx], cx+1, rowY+1.2, { size: 7, color: black }); } }
      const mC = ML+168, tC = ML+179;
      if (i < 3) { const mOk = tempos_opt[i] === rdo.condicao_tempo_manha; const aOk = tempos_opt[i] === rdo.condicao_tempo_tarde; text(mOk ? '■' : '□', mC+2, rowY+1.2, { size: 9, color: mOk ? orange : gray }); text(aOk ? '■' : '□', tC+2, rowY+1.2, { size: 9, color: aOk ? orange : gray }); }
      cx = ML; cols.forEach((w, ci) => { cx += w; line(cx, y, cx, y + nRows*rH, [203,213,225]); });
    }
    y += nRows * rH;
    ...
    slots.forEach((slot, idx) => {
      ...
      rect(ML, slotY, CW, slotH, null, [226,232,240]);
      const isLunch = slot === Utils.floorHour(rdo.horario_almoco);
      doc.setFontSize(8.5); doc.setFont('helvetica', 'bold'); doc.setTextColor(...(isLunch ? [217,119,6] : dark));
      doc.text(slot, ML+6, slotY+3, { baseline: 'top' });
      if (isLunch) { doc.setFont('helvetica', 'italic'); doc.setFontSize(7.5); doc.setTextColor(...gray); doc.text('Intervalo de almoço', ML+25, slotY+3, { baseline: 'top' }); }
      else {
        const linha = linhas.find(l => l.horario_ponto === slot);
        if (linha && linha.descricao_detalhada) {
          const txt = (linha.referencia_modulo ? `[${linha.referencia_modulo}] ` : '') + linha.descricao_detalhada;
          ...
          if (linha.status_atividade) {
            const sc = { 'Iniciada':[59,130,246],'Em Andamento':[234,179,8],'Concluída':[34,197,94] };
            const col = sc[linha.status_atividade] || gray;
            doc.setFillColor(...col); doc.roundedRect(PW-MR-24, slotY+2.5, 22, 4.5, 1, 1, 'F');
            doc.setFontSize(6); doc.setFont('helvetica','bold'); doc.setTextColor(...white);
            doc.text(linha.status_atividade, PW-MR-13, slotY+4, { align:'center', baseline:'top' });
          }
        }
    ...
    rect(ML, y, CW, 5, [248,250,252]); rect(ML, y, CW, 5, null, [226,232,240]);
    text('Observação: O RDO deve ser preenchido de hora em hora', ML+CW/2, y+1, { size: 7.5, color: gray, align: 'center' });
    y += 5;

    const obsH = 20;
    rect(ML, y, CW, 5.5, dark); text('CAMPO OBSERVAÇÕES E DESVIOS', ML+CW/2, y+1, { size: 8, bold: true, color: white, align: 'center' }); y += 5.5;
    rect(ML, y, CW, obsH, white, [203,213,225]);
    if (rdo.observacoes_desvios) { doc.setFontSize(8); doc.setFont('helvetica','normal'); doc.setTextColor(...black); doc.text(doc.splitTextToSize(rdo.observacoes_desvios, CW-4), ML+2, y+2, { baseline: 'top' }); }
    y += obsH;

    const fiscH = 18;
    rect(ML, y, CW, 5.5, dark); text('CAMPO DESTINADO A COMENTÁRIOS FISCALIZAÇÃO', ML+CW/2, y+1, { size: 8, bold: true, color: white, align: 'center' }); y += 5.5;
    rect(ML, y, CW, fiscH, white, [203,213,225]);
    if (rdo.comentarios_fiscalizacao) { doc.setFontSize(8); doc.setFont('helvetica','normal'); doc.setTextColor(...black); doc.text(doc.splitTextToSize(rdo.comentarios_fiscalizacao, CW-4), ML+2, y+2, { baseline: 'top' }); }
    y += fiscH;
    ...
    doc.line(ML+10, y+20, ML+CW/2-5, y+20);
    text('Modular', ML+CW/4, y+22, { size: 8, bold: true, color: dark, align: 'center' });
    if (rdo.assinatura_cliente_base64) { try { doc.addImage(rdo.assinatura_cliente_base64, 'PNG', ML+CW/2+5, y+3, 80, 16); } catch {} }
    doc.line(ML+CW/2+10, y+20, PW-MR-10, y+20);
    const sigLabel = rdo.assinatura_nome_confirmacao ? `Nome e Assinatura do Cliente: ${rdo.assinatura_nome_confirmacao}` : 'Nome e Assinatura do Cliente:';
    text(sigLabel, ML+CW*0.75, y+22, { size: 8, bold: true, color: dark, align: 'center' });

    if (rdo.fotos && rdo.fotos.length > 0) {
      if (y + 10 > PH - 20) { doc.addPage(); y = MT; }
      rect(ML, y, CW, 5.5, dark); text(`REGISTRO FOTOGRÁFICO (${rdo.fotos.length} foto${rdo.fotos.length > 1 ? 's' : ''})`, ML + CW / 2, y + 1, { size: 8, bold: true, color: white, align: 'center' }); y += 5.5;
      const cols4 = 4; const fotoW = (CW - (cols4 - 1) * 2) / cols4; const fotoH = fotoW * 0.75;
      rdo.fotos.forEach((b64, idx) => {
        const col = idx % cols4; const row = Math.floor(idx / cols4);
        const fx = ML + col * (fotoW + 2); const fy = y + row * (fotoH + 2);
        if (fy + fotoH > PH - 15) { doc.addPage(); y = MT; }
        try { const fmt = b64.startsWith('data:image/png') ? 'PNG' : 'JPEG'; doc.addImage(b64, fmt, fx, fy, fotoW, fotoH); rect(fx, fy, fotoW, fotoH, null, [203, 213, 225]); } catch {}
      });
      const fotoRows = Math.ceil(rdo.fotos.length / cols4); y += fotoRows * (fotoH + 2) + 2;
    }
